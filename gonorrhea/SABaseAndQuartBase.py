...
varBaseQuarterlyConditions = [
    Cls.VariableCondition('Decision Period', 91, 91, False),
    Cls.VariableCondition('% Resistant Threshold', 0.03, 0.1, True, label_format='{:.1%}'),
    Cls.VariableCondition('Change in % Resistant Threshold', 1, 1, False)
]
...
series = [
    Cls.Series('Base', 'blue', #'#1874CD',
               ...
               labels_shift_x=-10,
               labels_shift_y=0.04),
    Cls.Series('Base-Quarterly', 'red',# '#FF4040',
               variable_conditions=varBaseQuarterlyConditions,
               if_find_frontier=True,
               labels_shift_x=2.5,
               labels_shift_y=-0.11)
]
